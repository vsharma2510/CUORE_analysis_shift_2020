...
int main(int argc, char* argv[]){
  ...
  for(it=channel.begin();it!=channel.end();it++){
    ...
    cut_baseline.Form("Channel==%d",(*it)); //IsSignal?
    cut_heater.Form("Channel==%d",(*it));
    ...
    c1->cd();
    h_baseline->Draw();
    TString baseline_title = Form("baseline_vs_amplitude_ch%d_C_all.pdf",(*it));
  ...
  h_heater->Draw();
  TString heater_title = Form("heater_amplitude_ch%d_C_all.pdf",(*it));
