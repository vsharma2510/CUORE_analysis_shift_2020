...
int main(int argc, char* argv[]){
  ...
  for(it=channel.begin();it!=channel.end();it++){
    ...
    for(int i=0;i<4;i++){
      ...
      unsigned int sizeArr1=ch.Draw("Baseline:Time",cut_baseline.Data(),"GOFF");
	...
	Time[j]=time[j];
      ...
      TH2D* h_stab_time = new TH2D("stab_time","Baseline vs Time",1000,0,90000,1000,-10000,0);
      //h_baseline->FillN(sizeArr1,Baseline,0,Amplitude,1);
      for(int j=0;j<sizeArr1;j++){
	h_stab_time->Fill(Time[j],Stab[j]);
      }
      TString stab_histo_title = Form("Baseline vs Time {Channel==%d && IsStabPulser}",(*it));
      h_stab_time->GetXaxis()->SetTitle("Time");
      h_stab_time->GetYaxis()->SetTitle("Baseline");
      ...
      h_stab_time->Draw();
      TString stab_title = Form("baseline_vs_time_run%d_ch%d_C.pdf",run_number[i],(*it));
